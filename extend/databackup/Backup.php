...
// 
//  
//  @remark lastTime：2021年4月15日17:24:11
//  修改部分源码 移动到extend下(无需composer)
// 
...
class Backup
{
    ...
    public function fileList()
    {
        ...
        foreach ($glob as $name => $file) {
            if (preg_match('/^\\d{8,8}-\\d{6,6}-\\d+\\.sql(?:\\.gz)?$/', $name)) {
                ...
                $time_original = "{$name[3]}{$name[4]}{$name[5]}";
                $part = $name[6];
                ...
                $info['curtime'] = date('Y-m-d H:i:s',strtotime("{$date} {$time}"));
                //$list["{$date} {$time}"] = $info;
                $info['name'] = "{$date}-{$time_original}-1";
                $list[$i] = $info;
                $i++;
            }
        }
    ...
    {   
        //判断视图-跳过
        if(strstr($table, 'view_')){return false;}
        $db = self::connect();
